...
import com.nbna.emailing.EmailCustomer;
...
public class AppController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (url.endsWith("Register")){
			...
			try {
				boolean resultOfTransaction = ids.execute(request);
				if(resultOfTransaction)
					pw.write("Sucessfully created an account please look forward for email");
				else
					pw.write("Passwords did not match. Please reenter the passwords again");
 ...
 * 	2) You can still use the login information from current page to which ever page it
 *     is getting forwarded to
 *  3) You don't have to set values to a scope, you would just pass on the request 
 *  	and response.
 */
